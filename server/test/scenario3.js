...
describe('scenario 3', ()=> {
  ...
  it('should scan user 1 with user 2, 2 with 3', async ()=>{
    testSuite.scans.push({ my_key: testSuite.user1.scan_key,scan_key: testSuite.user2.scan_key, scan_time:Date.now() })
    testSuite.scans.push({ my_key: testSuite.user2.scan_key,scan_key: testSuite.user3.scan_key, scan_time:Date.now() })
  ...
  it('is true that user 3 has contact', async () => {
    const res = await api.state(testSuite.user3.scan_key);
    res.body.res.should.equal(1);
  })
