...
router.post('/', function (req, res) {
  ...
  var select_sql = "select * from scan where my_key = ? and scan_key = ?";
  var update_sql = "update scan set scan_time = ? where my_key = ? and scan_key = ?";

  var upsert_sql = "insert into scan (my_key, scan_key, scan_time) values (?,?,?) ON DUPLICATE KEY UPDATE scan_time = ?";

  var scan_key, scan_time;

  my_key = data[0].my_key;
  scan_key = data[0].scan_key;
  scan_time = data[0].scan_time;
  var param = [my_key, scan_key, scan_time, scan_time];
  db.mysql.query(upsert_sql, param, function (err, result) {
    if (err) {
      console.log(err);
      res.json({
        'res': '-1'
